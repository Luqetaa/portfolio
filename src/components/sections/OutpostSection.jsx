...
/* ────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────── */
const FRAME_CONFIGS = [
  { w: "92%", h: "88%", scaleRange: [1.5, 1.9, 2.6, 5.0], opacity: 0.2, border: 2 },
  { w: "75%", h: "70%", scaleRange: [1.3, 1.7, 2.3, 4.5], opacity: 0.3, border: 2 },
  { w: "55%", h: "50%", scaleRange: [1.1, 1.4, 2.0, 4.0], opacity: 0.4, border: 2 },
  { w: "35%", h: "30%", scaleRange: [0.9, 1.2, 1.7, 3.5], opacity: 0.6, border: 2 },
];

/* ── Pixel block masses — tactical map background ── */
function PixelBlocks({ color }) {
...
function FrameLayer({ cfg, scrollYProgress, isMobile }) {
  const { w, h, scaleRange, opacity, border } = cfg;

  // Map progress to an exponential curve to mimic 3D travel
  // Finish zooming by 0.3 so the projects can appear
  const scale = useTransform(scrollYProgress, [0, 0.1, 0.2, 0.3], scaleRange);
...
function CenterContent({ accentColor, scrollYProgress, theme }) {
  ...
  const contentY = useTransform(scrollYProgress, [0, 0.15, 0.25], [0, 0, -50]);
...
function CinematicProjectList({ scrollYProgress, accentColor }) {
  ...
  const containerOpacity = useTransform(scrollYProgress, [0.25, 0.35, 0.9, 1], [0, 1, 1, 0]);
  
  // Parallax upward movement so users can scroll through the items
  const containerY = useTransform(scrollYProgress, [0.3, 1], ["10vh", "-200vh"]);
